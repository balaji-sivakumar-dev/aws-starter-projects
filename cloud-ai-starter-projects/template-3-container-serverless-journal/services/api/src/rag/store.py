...
class DynamoDBVectorStore(VectorStore):
    ...
    Items stored with:
      PK = 'VEC#<tenant_id>'  (never collides with 'USER#' journal entries)
      SK = 'DOC#<doc_id>'

    Cosine similarity is computed in Lambda memory after querying all tenant
    vectors.  Suitable for personal journals (< 5 000 entries per user).

    Required env var: JOURNAL_TABLE_NAME
    ...
    def __init__(self) -> None:
        ...
        self.table_name = os.getenv("JOURNAL_TABLE_NAME", "")
        if not self.table_name:
            raise ValueError("JOURNAL_TABLE_NAME is required for DynamoDB vector store")
