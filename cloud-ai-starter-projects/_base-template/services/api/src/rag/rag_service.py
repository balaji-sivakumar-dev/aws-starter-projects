"""
RAG service — retrieval-augmented generation for data queries.

Combines vector retrieval with LLM completion to answer user questions
about their items with source citations.
"""
...
class RAGService:
    """Orchestrates retrieval + LLM generation for conversational data queries."""
    ...
    def ask(self, tenant_id: str, query: str, top_k: int = 5, provider_name: str | None = None) -> RAGAnswer:
        """
        Answer a user's question using their items as context.
        ...
        if not results:
            return RAGAnswer(
                answer="I don't have enough items to answer that question. "
    ...
    def _generate_answer(self, query: str, context: str, provider_name: str | None = None) -> str:
        ...
        prompt = (
            "You are a personal assistant. The user is asking a question about "
            "their own items. Use ONLY the items provided below to "
        ...
        if not raw:
            try:
                ...
                raw = ("Based on the items I found, here are the relevant passages. "
