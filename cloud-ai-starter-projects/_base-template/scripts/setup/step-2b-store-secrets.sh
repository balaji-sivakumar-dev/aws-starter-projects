...
#
# Usage:
#   AWS_PROFILE={{APP_PREFIX}}-dev ./scripts/setup/step-2b-store-secrets.sh dev
#
...
#   /{{APP_PREFIX}}/<env>/cognito/allowed_emails   — all emails (pre-signup Lambda allowlist)
#   /{{APP_PREFIX}}/<env>/admin_emails             — admin emails only (Lambda ADMIN_EMAILS env var)
#   /{{APP_PREFIX}}/<env>/groq_api_key             — Groq API key (only if LLM_PROVIDER=groq)
#   /{{APP_PREFIX}}/<env>/openai_api_key           — OpenAI API key (only if LLM_PROVIDER=openai or EMBEDDING_PROVIDER=openai)
...
store_param \
  "/{{APP_PREFIX}}/${ENV}/cognito/allowed_emails" \
  "$ALL_EMAILS" \
  "Comma-separated emails allowed to register in {{APP_PREFIX}}-${ENV}"

# ── Store admin emails ────────────────────────────────────────────────────────
if [[ -n "$ADMIN_EMAILS" ]]; then
  store_param \
    "/{{APP_PREFIX}}/${ENV}/admin_emails" \
    "$ADMIN_EMAILS" \
    "Comma-separated admin emails for {{APP_PREFIX}}-${ENV} (Lambda ADMIN_EMAILS env var)"
else
...
if [ "$LLM_PROVIDER" = "groq" ]; then
  ...
  if [ -z "${GROQ_API_KEY:-}" ]; then
    ...
    store_param \
      "/{{APP_PREFIX}}/${ENV}/groq_api_key" \
      "$GROQ_API_KEY" \
      "Groq API key for {{APP_PREFIX}}-${ENV} AI enrichment"
  fi
  ...
  if [ -z "${OPENAI_API_KEY:-}" ]; then
    ...
    store_param \
      "/{{APP_PREFIX}}/${ENV}/openai_api_key" \
      "$OPENAI_API_KEY" \
      "OpenAI API key for {{APP_PREFIX}}-${ENV} (LLM and/or embeddings)"
  fi
else
