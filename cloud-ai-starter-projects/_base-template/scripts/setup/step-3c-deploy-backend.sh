...
#   AWS_PROFILE={{APP_PREFIX}}-dev ./scripts/setup/step-3c-deploy-backend.sh dev
#
# Optional — deploy only a subset:
#   DEPLOY_API=false    AWS_PROFILE={{APP_PREFIX}}-dev ./scripts/setup/step-3c-deploy-backend.sh dev
#   DEPLOY_AI=false     AWS_PROFILE={{APP_PREFIX}}-dev ./scripts/setup/step-3c-deploy-backend.sh dev
#   DEPLOY_SFN=false    AWS_PROFILE={{APP_PREFIX}}-dev ./scripts/setup/step-3c-deploy-backend.sh dev
#
# When API Gateway routes have been added/changed (main.tf api_routes map changed):
#   DEPLOY_ROUTES=true  AWS_PROFILE={{APP_PREFIX}}-dev ./scripts/setup/step-3c-deploy-backend.sh dev
...
ADMIN_EMAILS_SSM=$(aws ssm get-parameter \
  --name "/{{APP_PREFIX}}/${ENV_NAME}/admin_emails" \
  --with-decryption \
...
if [[ -n "${ADMIN_EMAILS_SSM}" ]]; then
  ...
  echo ">> Admin emails: none found in SSM (/{{APP_PREFIX}}/${ENV_NAME}/admin_emails)"
...
echo "   AWS_PROFILE=${AWS_PROFILE:-{{APP_PREFIX}}-dev} ./scripts/setup/step-4a-deploy-web-to-s3.sh ${ENV_NAME}"
