...
#
# Usage:
#   AWS_PROFILE={{APP_PREFIX}}-dev ./scripts/setup/step-2c-create-cognito-admin.sh dev
#
# Options (env vars):
...
aws ssm put-parameter \
  --name "/{{APP_PREFIX}}/${ENV}/admin_user_ids" \
  --value "$SUBS_CSV" \
  --type "SecureString" \
  --description "Cognito sub UUIDs for admin users in {{APP_PREFIX}}-${ENV}" \
  --overwrite \
  --profile "$PROFILE" \
  --output text > /dev/null

echo "  ✅ Stored in SSM: /{{APP_PREFIX}}/${ENV}/admin_user_ids"
echo "  Value: $SUBS_CSV"
echo ""
