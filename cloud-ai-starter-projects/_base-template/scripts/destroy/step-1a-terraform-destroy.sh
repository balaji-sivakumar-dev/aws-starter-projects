...
COGNITO_PLACEHOLDER="{{APP_PREFIX}}-${ENV_NAME}-change-me"
...
echo "  1. AWS_PROFILE=${AWS_PROFILE:-{{APP_PREFIX}}-dev} ./scripts/setup/step-2b-store-secrets.sh ${ENV_NAME}"
echo "  2. AWS_PROFILE=${AWS_PROFILE:-{{APP_PREFIX}}-dev} ./scripts/setup/step-3a-terraform-apply.sh ${ENV_NAME}"
echo "  3. AWS_PROFILE=${AWS_PROFILE:-{{APP_PREFIX}}-dev} ./scripts/setup/step-4a-deploy-web-to-s3.sh ${ENV_NAME}"
